...
/**
 * Convert a BotSailor template body_content into a regex pattern that
 * matches any filled instance of that template.
 *
 * Handles three placeholder styles:
 *   {{1}}, {{2}}, ...         — Meta-style numbered variables
 *   #!system-...!#            — BotSailor system variables
 *   #LEAD_USER_FIRST_NAME#    — BotSailor lead variables
 */
function templateBodyToPattern(body: string): string {
  //
  // Step 1: Escape regex special characters from the literal text.
  // We do this per-segment so we don't accidentally escape the
  // placeholder replacements we're about to insert.
  //
  // Split on placeholder patterns, escape the literal parts, then
  // rejoin with wildcard groups.
  //
  ...
  const patternParts = parts.map(part => {
    // {{1}}, {{2}}, etc.
    if (/^\{\{\d+\}\}$/.test(part)) return '([\\s\\S]*?)'
    // #!system-cart-product-list!# etc.
    if (/^#![A-Za-z0-9_-]+!#$/.test(part)) return '([\\s\\S]*?)'
    // #LEAD_USER_FIRST_NAME# etc.
    if (/^#[A-Z_]+#$/.test(part)) return '([\\s\\S]*?)'
    // Literal text — escape regex specials
...
/**
 * Normalize text for matching: strip emoji, collapse whitespace.
 */
...
function matchTemplateButtons(
  ...
  for (const tmpl of templates) {
    ...
    const pattern = templateBodyToPattern(normalizedBody)
    try {
